...
const PRIORITY_COMMANDS = ['deploy', 'dev', 'env', 'init', 'login'];
...
for (const cmd of PRIORITY_COMMANDS) {
  ...
  if (existsSync(srcPath)) {
    ...
    content = content.replace(/import\("\.\/chunks\//g, 'import("../../chunks/');
...
copyFileSync(
  ...
  new URL('VERCEL_DIR_README.txt', new URL('dist/', repoRoot))
);
copyFileSync(
  new URL('src/util/dev/builder-worker.cjs', repoRoot),
  new URL('builder-worker.cjs', new URL('dist/', repoRoot))
);
copyFileSync(
  new URL('src/util/get-latest-version/get-latest-worker.cjs', repoRoot),
  new URL('get-latest-worker.cjs', new URL('dist/', repoRoot))
);
copyFileSync(
  new URL('src/vc.js', repoRoot),
  new URL('vc.js', new URL('dist/', repoRoot))
);

// Generate version.mjs for fast --version lookup
writeFileSync(
  new URL('version.mjs', new URL('dist/', repoRoot)),
  `export const version = ${JSON.stringify(pkg.version)};\n`
);

// Write metafile and check for duplicates if requested
if (result.metafile) {
  const metafilePath = join(cwd, 'metadata.json');
